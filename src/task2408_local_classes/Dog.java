    ...
    public CanSpeak toCanSpeak(final int i) {
        class DogPet extends SuperDog implements CanSpeak {
            ...
            private String getName() {
                sb.append(this.getSuperQuotes()).append(Dog.this.name).append(this.getSuperQuotes());
                return sb.toString();
            }
            @Override
            public String speak() {
                if (i < 1) {
                    ...
                    getName();
                    sb.append(" says r");
                    for (int j = 0; j < i; j++) sb.append("u");
                    sb.append("ff! ").append(formatter.format(new Date()));
                    return sb.toString();
                }
            }
        }
